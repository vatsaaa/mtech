...
- Hyperplane: Decision boundary that separates the data into different classes
...
- +ve class: w^T * x + b >= 1
- -ve class: w^T * x + b <= -1
- Margin: 2 / ||w||, by maximizing the margin, we minimize the generalization error
- Optimization: Minimize 1/2 * ||w||² subject to y_i * (w^T * x_i + b) >= 1
...
- Prediction: Predict the class of a new data point based on the sign of w^T * x + b
...
- The prediction is made based on the sign of w^T * x + b
  - w^T * x + b >= 0, the data point belongs to one class
  - w^T * x + b < 0, the data point belongs to the other class
- Lagrangian function: L(w, b, α) = 1/2 * ||w||² - Σα_i * (y_i * (w^T * x_i + b) - 1)
- Dual optimization: Maximize Σα_i - 1/2 * ΣΣα_i * α_j * y_i * y_j * (x_i^T * x_j)
- The optimization problem is solved using the Lagrange multipliers
- Solve for w, b, and α
- Find the equation of the hyperplane: w^T * x + b = 0
- Predict the class of a new data point based on the sign of w^T * x + b

### Slack Variables, Soft and Hard Margin SVM

- Soft Margin SVM (C = C = 1 / λ, where λ is the regularization strength parameter): Allows for a certain number of misclassifications
