...
def gradient_descent(x, y, alpha, beta, c, num_iterations):
    for i in range(num_iterations):
        print("Iteration ", i+1, ": ")
...
def main():
    ...
    if args.question == 'q1':
        ...
        A1 = np.dot(MC.T, MC)
        # A1 = np.array([[1210, 939, 1146], [939, 861, 1000], [1146, 1000, 1209]])

        print("Matrix A1 = C^TC:\n", A1)

        eigenvalues, eigenvectors = np.linalg.eig(A1)
        characteristic_equation = np.polynomial.polynomial.polyfromroots(eigenvalues)
        ...
        """
        largest_eigenvalue, eigenvector = power_method(A1, 10)
        print("Largest Eigenvalue λ1:", largest_eigenvalue)
        print("Corresponding Eigenvector:")
        pprint(eigenvector)
        print("Normalized Eigenvector x1_cap:")
        pprint(eigenvector / np.linalg.norm(eigenvector))
        ...
        """
        A2 = A1 - np.dot(np.dot(eigenvector, eigenvector.T), A1)
        largest_eigenvalue_A2, eigenvector_A2 = power_method(A2, 10)
        print("First 10 iterates give λ2:", largest_eigenvalue_A2)
        print("Corresponding eigenvector x2_cap:")
        pprint(eigenvector_A2)
        print("Normalized Eigenvector x2_cap:")
        pprint(eigenvector_A2 / np.linalg.norm(eigenvector_A2))
        ...
        """
        A3 = A1 - np.dot(np.dot(eigenvector, eigenvector.T), A1) - np.dot(np.dot(eigenvector_A2, eigenvector_A2.T), A1)
        largest_eigenvalue_A3, eigenvector_A3 = power_method(A3, 10)
        print("First 10 iterates give λ3:", largest_eigenvalue_A3)
        print("Corresponding Eigenvector of A3:")
        pprint(eigenvector_A3)
        print("Normalized Eigenvector x3_cap:")
        pprint(eigenvector_A3 / np.linalg.norm(eigenvector_A3))

        print("Comparison of λ1, λ2, and λ3:")
        print("λ1:", largest_eigenvalue, "λ2:", largest_eigenvalue_A2, "λ3:", largest_eigenvalue_A3)
        ...
        x_star, y_star, alpha = gradient_descent(x0, y0, alpha, beta, c, num_iterations)
